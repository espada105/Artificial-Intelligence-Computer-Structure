...
int main(){
    ...
    //현재 GPU의 "컴퓨팅 기능"을 나타내는 주 버전(major)을 가져옴
    
    int computeCapabilityMinor = props.minor; 
    //현재 GPU의 "컴퓨팅 기능"을 나타내는 부 버전(minor)을 가져옴
    
    int multiprocessorCount = props.multiProcessorCount; 
    //  GPU에 있는 스트리밍 멀티프로세서(SM)의 개수를 가져옴
    
    int warpSize = props.warpSize; 
    // 워프 크기(즉, 한 번에 몇 개의 스레드가 그룹으로 처리되는지)를 가져옴. 일반적으로 이 값은 32
